...
`$env:USERPROFILE\.agents` 与 WSL 的 `~/.agents` 或 `/mnt/c/...` 路径混用。
...
Sol 先判断任务的风险、边界和独立验证方式。凡是低风险、范围清晰、无需架构判断、无需用户授权、可以直接执行和验证的“小活”或无技术含量的机械工作，默认直接交给 `luna_worker`，即使只涉及一个文件、预计少于 2 分钟，也不要由 Sol 亲自实现。典型例子包括简单 CRUD、格式调整、字段增删改、模板套用、重复性重命名、明确的配置修改和已有方案下的局部修复。

除上述“小活”外，满足任一条件时，也默认将可限定的落地工作交给一个 Luna：涉及两个以上文件、预计超过 2 分钟、需要重复机械操作、需要独立测试或对抗审查，或存在清晰的文件/目录边界。

Sol 不委派需求理解、架构设计、跨模块取舍、最终验收、commit、PR、发布或需要用户授权的破坏性操作。纯问答、单条只读命令和简单状态查询可由 Sol 直接处理；“单文件几行以内”不再自动构成 Sol 直做的例外，只要属于低风险、无技术含量、可直接执行的小活，就应派给 `luna_worker`。委派不改变 Sol 的最终决策权，也不把责任转移给 Luna。
...
当已派出的子智能体存在真实运行中的任务（正在执行命令、安装、编译、测试或等待外部进程完成）时，Sol 必须持续等待到任务结束、明确失败或需要用户输入；不得因暂时没有新输出而提前宣称完成，也不得把仍在运行的任务当作空跑而中断。等待期间可发送状态查询，但应使用有界等待并在状态变化后继续跟进。

当多个子任务的输入、输出、写入文件和依赖彼此独立，且不存在共享状态或顺序要求时，Sol 应安排多个不冲突的子智能体并行执行；每个并行任务仍须有独立任务卡、验收标准和闸门证据。若任务共享文件、共享生成物、安装器锁或前置结论，则必须改为串行。

Sol 只有在所有已派发的并行任务均已结束并完成独立验收后，才能汇总并向用户输出最终结果。任一任务失败、超时、产物缺失或需要用户输入时，整体结果必须明确标记未完成，并锁定依赖该任务的后续阶段。
