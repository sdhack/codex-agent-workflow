...
Sol/Luna 分层代理工作流：Sol 负责目标、架构、调度与最终验收，Luna 按任务卡负责可验证的探索、实现和验证。
...
默认流程为：盘点 → 实现 → 验证 → Sol 验收。审查（`luna_critic`）是按需安排的可选阶段，不属于默认 DAG；若安排，必须单独记录依赖和闸门。每个 Luna 任务必须限定目标、文件范围、验收和回报；Luna 不改变架构、不提交、不发布、不处理密钥，Sol 独立检查 diff、测试、日志和产物。阶段失败最多回派两次，证据不足不得放行。

当已派出的子任务正在执行命令、安装、编译、测试或等待外部进程时，Sol 必须持续等待到 `completed`、`failed`、`timed_out` 或 `needs_user_input`；正常等待不应被判作 stalled。输入、输出、写入范围和依赖均独立的任务可以并行，最多 3 个；每个并行任务都要有独立任务卡和闸门。只有所有任务结束且 Sol 独立验收通过后才能统一输出；任一失败、超时、缺少关键产物或需要用户输入，整体即为未完成并锁定依赖阶段。
